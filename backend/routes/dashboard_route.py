...
def get_widget_data(widget_id):
    try:
        ...
        if response_data is None:
             return jsonify({'message': 'Widget nebyl nalezen'}), 404
...
def get_dashboard_widgets(dashboard_id):
    ...
    if widgets_data is None:
        return jsonify({'message': 'Dashboard not found or access denied'}), 404
...
def update_widget(widget_id):
    ...
    if not result:
        return jsonify({'message': 'Widget not found or access denied'}), 404
