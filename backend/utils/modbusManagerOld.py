...
import serial
import time
from models.models import Sensor, SensorData, db, UserSensor
...
class ModbusManager_2_0:
    def __init__(self, 
                 cache_size: int = 10,           # Maximální počet záznamů v cache
                 batch_size: int = 5,           # Velikost batch pro DB zápis
                 flush_interval: float = 120,    # Interval automatického ukládání (sekundy)
        ...
        # Spustíme background workery a načteme všechny senzory
    ...
    def load_all_sensors(self) -> bool:
        """Načte všechny senzory z databáze a spustí jejich automatické čtení."""
        with self.app.app_context():
            try:
                sensors = Sensor.query.all()
                if not sensors:
                    current_app.logger.error("No sensors found in database")
                    return False
                ...
                # Inicializace modbus instrumentu
                if self.instrument is None:
                    self._init_modbus()
                
                # Spustíme automatické čtení pro všechny senzory
                self._start_all_sensors_reading()
                return True
            except Exception as e:
                current_app.logger.error(f"Failed to load sensors: {e}")
                return False

    def load_user_sensors(self, user_id: int) -> bool:
        """Načte senzory pro daného uživatele do user_sensors, čtení již běží."""
        with self.app.app_context():
            try:
                userSensors = self._load_user_sensors(user_id)
                if not userSensors:
                    current_app.logger.error(f"No sensors found for user {user_id}")
                    return False
                sensors = []
                for mapp in userSensors:
                    sensor = self.sensor_map.get(mapp.sensor_id)
                    if sensor:
                        sensors.append(sensor)
                    else:
                        current_app.logger.error(f"Sensor {mapp.sensor_id} not found in the system.")
                
                self.user_sensors[user_id] = sensors
                current_app.logger.info(f"Loaded {len(sensors)} sensors for user {user_id}")
                return True
            except Exception as e:
                current_app.logger.error(f"Error loading sensors for {user_id}: {e}")
                return False

    def _load_user_sensors(self, user_id: int) -> List[UserSensor]:
        """Načte senzory z sensor_map pro daného uživatele."""
        with self.app.app_context():
            try:
                return  UserSensor.query.filter_by(user_id=user_id).all()
            except Exception as e:
                current_app.logger.error(f"Error filtering sensors for user {user_id}: {e}")
                return []
    ...
    def _start_all_sensors_reading(self):
        """Spustí vlákno pro automatické čtení všech senzorů."""
        with self.app.app_context():
            if self.sensor_map and not hasattr(self, '_all_sensors_thread'):
                self._all_sensors_event = threading.Event()
                thread = threading.Thread(
                    target=self._auto_read_all_sensors,
                    daemon=True
                )
                self._all_sensors_thread = thread
                thread.start()
                current_app.logger.info("Started automatic reading for all sensors")

    def _auto_read_all_sensors(self):
        """Automaticky čte data ze všech senzorů podle jejich intervalů."""
        last_read_time = {sensor_id: 0.0 for sensor_id in self.sensor_map}
            ...
            with self.app.app_context():
                try:
                    ...
                    for sensor in self.sensor_map.values():
                        if sensor.is_active and (current_time - last_read_time[sensor.sensor_id]) >= sensor.sampling_rate * 10:
                            value = self.read_sensor(sensor.sensor_id)
                            sensor_data[sensor.sensor_id] = value
                            last_read_time[sensor.sensor_id] = current_time
                        if sensor_data:
                            current_app.logger.debug(f"Automatically read sensors: {sensor_data}")
                except Exception as e:
                    current_app.logger.error(f"Error in auto reading sensors: {e}")
                time.sleep(1)

    def _auto_read_sensors(self, user_id: int):
        """Spustí vlákno pro automatické čtení senzorů uživatele (pouze pro kompatibilitu)."""
        with self.app.app_context():
            current_app.logger.info(f"User-specific reading thread for user {user_id} not needed, using global sensor reading")
            return
    ...
    def get_cached_data(self, sensor_id: int, max_age_minutes: int = 5) -> Optional[Dict]:
        ...
        with self.app.app_context():
            cutoff_time = datetime.now(local_tz) - timedelta(minutes=max_age_minutes)
    ...
    def shutdown(self):
        ...
        with self.app.app_context():
            ...
            # Zastavíme čtení všech senzorů
            if hasattr(self, '_all_sensors_event'):
                self._all_sensors_event.set()
                if hasattr(self, '_all_sensors_thread'):
                    self._all_sensors_thread.join(timeout=1.0)
            
            # Zastavíme uživatelská čtení (pro kompatibilitu)
        ...
        with self.app.app_context():
            if user_id in self.user_read_events:
                ...
                if user_id in self.user_read_threads:
                    self.user_read_threads[user_id].join(timeout=1.0)
    ...
    def release_sensors(self, user_id: int):
        """Uvolní senzory pro daného uživatele."""
        with self.app.app_context():
            # Zastavíme čtení senzorů (pro kompatibilitu)
            self._stop_user_reading(user_id)
