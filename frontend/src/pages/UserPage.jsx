...
const UserPage = () => {
  ...
  return (
    <div className="main-content">
      ...
      {!loading && !error && actionError && <div className="error-message">{actionError}</div>}

      {!loading && !error && (
        <>
          <div className="users-toolbar">
            <button className="create-user-btn" onClick={() => navigate("/insertUser")}>
              Vytvořit uživatele
            </button>
          </div>
                      ...
                      <td className="user-actions" onClick={(e) => e.stopPropagation()}>
                        {isEditing ? (
                          <>
                            ...
                            <button className="user-btn cancel-btn" onClick={cancelEdit} disabled={saving}>
                              Zrušit
                            </button>
                          </>
                        ) : (
                          <>
                            <button className="user-btn edit-btn" onClick={() => startEdit(user)}>
                              Upravit
                            </button>
                            <button
                            ...
                            >
                              {deletingUserId === user.user_id ? "Mazání..." : "Smazat"}
                            </button>
                          </>
                        )}
                      </td>
                      ...
                      <tr className="sensor-row-wrapper">
                        <td colSpan={7}>
                          {loadingSensorsUserId === user.user_id ? (
                            <div className="sensor-loading">Načítání senzorů...</div>
                          ) : sensorsForUser.length === 0 ? (
                            <div className="sensor-loading">Uživatel nemá žádné senzory.</div>
                          ) : (
                            <table className="sensor-subtable">
                              ...
                              <tbody>
                                {sensorsForUser.map((sensor) => {
                                  ...
                                  const isEditingSensor = editingSensorKey === sensorKey;
                                  return (
                                      ...
                                      <td>
                                        {isEditingSensor ? (
                                          <input
                                            ...
                                            onChange={handleSensorEditChange}
                                          />
                                        ) : (
                                          sensor.name
                                        )}
                                      ...
                                      <td>
                                        {isEditingSensor ? (
                                          <input
                                            className="table-input"
                                            name="sensor_type"
                                            value={sensorEditForm.sensor_type}
                                            onChange={handleSensorEditChange}
                                          />
                                        ) : (
                                          sensor.sensor_type
                                      ...
                                      <td>
                                        {isEditingSensor ? (
                                          <input
                                            ...
                                            name="sampling_rate"
                                            type="number"
                                            value={sensorEditForm.sampling_rate}
                                            onChange={handleSensorEditChange}
                                          />
                                        ) : (
                                      ...
                                      <td>
                                        {isEditingSensor ? (
                                          <input
                                            type="checkbox"
                                            name="is_active"
                                            checked={sensorEditForm.is_active}
                                            onChange={handleSensorEditChange}
                                          />
                                        ) : sensor.is_active ? (
                                          "Ano"
                                      ...
                                      <td className="user-actions">
                                        {isEditingSensor ? (
                                          <>
                                            <button
                                              className="user-btn save-btn"
                                              onClick={() => saveUserSensor(user.user_id, sensor.sensor_id)}
                                              disabled={savingSensorKey === sensorKey}
                                            >
                                              Uložit
                                            </button>
                                            <button className="user-btn cancel-btn" onClick={cancelSensorEdit}>
                                              Zrušit
                                            </button>
                                          </>
                                        ) : (
                                          <button
                                            ...
                                            onClick={() => startSensorEdit(user.user_id, sensor)}
                                          >
                                            Upravit
                                          </button>
                                        )}
